...
{
    QString stringa = QString("TITAN CAN DRIVER SERVER ADDRESS: IP:%1, PORT:%2 ").arg(localip.toString()).arg(localport);
    qDebug() << stringa;
    ...
    for(int i =0; i< socketList.size(); i++){
        if((socketList[i]->canId == canId) || (socketList[i]->canId == 0)){
            if(socketList[i]->canId == 0){
                QString stringa = QString("CAN: %1 %2 %3 %4 %5 %6 %7 %8").arg((uchar) frame[3]).arg((uchar) frame[4]).arg((uchar) frame[5]).arg((uchar) frame[6]).arg((uchar) frame[7]).arg((uchar) frame[8]).arg((uchar) frame[9]).arg((uchar) frame[10]);
