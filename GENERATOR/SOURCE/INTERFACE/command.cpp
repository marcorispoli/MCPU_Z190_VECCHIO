 ...
 * pre pulse exposure data when the next exposure is of AEC type (2D or 3D).
 *
...
QList<QString> Server::SetExposurePre(QList<QString>* command){
    ...
    statusManager::tExposureFocus focus = statusManager::_FOCUS_SMALL;
    if(command->at(5) == "LARGE") focus = statusManager::_FOCUS_LARGE;

    STATUS->setPreData(focus, command->at(3).toFloat(), command->at(4).toFloat(), command->at(6).toUShort());
    STATUS->setExposureOptions(true, true);
...
QList<QString> Server::SetExposurePulse(QList<QString>* command){
    ...
    STATUS->setPulseData(focus, command->at(3).toFloat(), command->at(4).toFloat());
    STATUS->setExposureOptions(detector, grid);
    answer.append("OK 0");
    return answer;
}
...
QList<QString> Server::SetTomoConfig(QList<QString>* command){
    ...
    STATUS->setTomoConfig(command->at(3).toUShort(), command->at(4).toUShort(), command->at(5).toUShort());
    answer.append("OK 0");
    return answer;
}
...
QList<QString> Server::GetPostExposure(QList<QString>* command){
    ...
    // The Exposure list should already be sent by the Generator device thrugh the R2CP protocol
    ...
    for(int i=0; i< list.size(); i++){
        if(list.at(i).foc)  answer.append("LARGE");
