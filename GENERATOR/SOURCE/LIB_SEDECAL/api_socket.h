	...
	namespace RTCS
	{
		...
		public:
			...
			*/
			 ...
			 * \return      (false)  if an error occurs.
			 */
			int16_t Accept(uint16_t localPort, uint8_t maxConn,int16_t timeout_rcv = 0);
			 ...
			 */
			int16_t Connect(uint16_t remotePort,dword remoteaddr,dword rx_Size, dword tx_Size,byte nattemps = NCONNECTATTEMPS);
			/*!
			 * \brief Shutdown
			 */
			void Shutdown( void );
			...
			*/	
			 ...
			 */
			int16_t Send(byte *buf,word len);
			 ...
			 * \return      (false)  if an error occurs. 
			 */
			virtual void OnReceive(byte* buf, dword len) = 0;
			 ...
			 */
			virtual void OnClose(/*asyncEvents*/byte closeEvent) = 0;
			/*
		...
		private:
			 ...
			 */
			friend void AcceptTask(dword pParam);
			/*!
			 *	\brief Read Task to parser the incoming messages 
			 */
			friend void ReadTask(dword pParam);
			#elif	WIN32
			 ...
			 */
			int16_t				m_timeout_rcv;
#if 0
			/*!
			 * \brief Mutex to block send request
			 */
			OS::mutex_handle_t  m_send_socket_mutex;
#endif
			/*!
			 * \brief Decoder types
			 */
