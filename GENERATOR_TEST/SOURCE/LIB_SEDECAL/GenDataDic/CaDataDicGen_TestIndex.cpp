...
namespace R2CP
{
	...
	void CaDataDicGen::Generator_Test_Handswitch_FootSwitch ( tDataDicAccess Access, byte *pData, word nData,  tInfoMessage *MessageInfo)
	{
		...
		if( m_p_TestInterface_ ) 
		{
			switch( Access )
			{
				case DATADIC_ACCESS_SET:									
					...
					pDataCp[1] = m_p_TestInterface_->II_Test_SS_HandSwitch_FootSwitch( pData[ 0 ] , pData[ 1 ] , pData[ 2 ] , pData[ 3 ] , pData [ 4 ] );
					break;
				case DATADIC_ACCESS_GET:					
					m_p_instance_->SetNodeEvent( MessageInfo->Node_dest , true );
					pDataCp[1] = m_p_TestInterface_->II_Test_SS_HandSwitch_FootSwitch();
					break;
			}
		}
		...
		(void)m_Type_->Processed( ETH_LOWEST_PRIORITY ,
								   ...
								   GENERATOR_TEST_HANDSWITCH_FOOTSWITCH, 
								   sizeof(pDataCp), 
								   (pDataCp));
	}
	
	void CaDataDicGen::Generator_Test_Error ( tDataDicAccess Access, byte *pData, word nData,  tInfoMessage *MessageInfo)
	{
		if(MessageInfo == nullptr)
			return;
	...
	void CaDataDicGen::Generator_Test_PowerControl ( tDataDicAccess Access, byte *pData, word nData,  tInfoMessage *MessageInfo )
	{
		if(MessageInfo == nullptr)
			return;
	...
	void CaDataDicGen::Generator_Test_Uarc ( tDataDicAccess Access, byte *pData, word nData,  tInfoMessage *MessageInfo )
	{
		if(MessageInfo == nullptr)
			return;
	...
	void CaDataDicGen::Generator_Test_Enable_Log ( tDataDicAccess Access, byte *pData, word nData,  tInfoMessage *MessageInfo )
	{
		if(MessageInfo == nullptr)
			return;
