...
namespace Api
{
	  ...
	  void 		(*Log::fcb_LogByEthernet_)(byte,char*) = nullptr;
	  ...
	  void Log::Print(Log_level error_level, const char *string , ...)
	  {
			...
			{
				fcb_LogByEthernet_( static_cast<byte>(error_level) , cadena_final_ );
			}
			
			LogMutex = false;
	  }
	
	  void Log::Set_System_Level( Log_level new_level , Output output , void (*LogByEthernet)(byte , char*))
	  {
