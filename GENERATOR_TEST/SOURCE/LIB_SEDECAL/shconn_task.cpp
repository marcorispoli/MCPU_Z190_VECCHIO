...
int16_t SendEthMsg(byte *msg, word n_msg)
{	
...
void CaSh_Connection::OnClose(byte closeEvent)
{
...
void CaSh_Connection::OnError(byte errorEvent)
{
...
bool CaSh_Connection::OnAccept(dword serviceSocket)
{
	return true;
}

void CaSh_Connection::OnReceive(byte* buf, dword len)
{	
	if( R2CP_Eth )
	{
